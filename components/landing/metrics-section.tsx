...
const metrics = [
  { 
    ...
    sublabel: "across Africa's schools",
  },
  { 
    value: 98, 
    suffix: "%", 
    ...
    sublabel: "with real-time updates",
  },
  { 
    value: 150, 
    suffix: "", 
...
export function MetricsSection() {
  ...
  return (
    <section ref={sectionRef} className="relative py-32 lg:py-40 bg-gradient-to-br from-slate-50 to-blue-50 overflow-hidden">
      <GridBackground />
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-12 gap-8 mb-20 lg:mb-32">
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex items-center gap-4 mb-6">
              <span className="flex items-center gap-2 px-3 py-1 bg-emerald-500/10 text-emerald-600 text-xs font-mono">
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[140px] font-display tracking-tight leading-[0.95] text-slate-900 transition-all duration-1000 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              Platform
              <br />
              <span className="text-blue-600">usage metrics.</span>
            </h2>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {/* Large metric */}
          <div className={`lg:col-span-1 bg-white border border-slate-200 p-10 lg:p-14 rounded-xl transition-all duration-700 ${
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
          }`}>
            <div className="text-4xl md:text-5xl lg:text-6xl font-display tracking-tight mb-4 whitespace-nowrap overflow-hidden text-blue-700">
              <AnimatedNumber end={metrics[0].value} suffix={metrics[0].suffix} prefix={metrics[0].prefix} />
            </div>
            <div className="mb-6">
              <DotGraph color="blue" height={36} freq1={0.28} freq2={0.09} freqT={0.5} speed={0.018} baseline={0.35} amplitude={0.55} />
            </div>
            <div className="text-lg text-slate-900 mb-2">{metrics[0].label}</div>
            <div className="text-sm text-slate-600 font-mono">{metrics[0].sublabel}</div>
          </div>
          ...
          {metrics.slice(1).map((metric, index) => (
            <div
              key={index}
              className={`bg-white border border-slate-200 p-8 lg:p-10 rounded-xl transition-all duration-700 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
              style={{ transitionDelay: `${200 + index * 100}ms` }}
            >
              <div className="text-3xl md:text-4xl lg:text-5xl font-display tracking-tight mb-3 whitespace-nowrap text-emerald-600">
                <AnimatedNumber end={metric.value} suffix={metric.suffix} prefix={metric.prefix} />
              </div>
              <div className="mb-4">
                <DotGraph color="green" height={24} />
              </div>
              <div className="text-base text-slate-900 mb-1 font-semibold">{metric.label}</div>
              <div className="text-xs text-slate-600 font-mono">{metric.sublabel}</div>
            </div>
          ))}
        </div>
